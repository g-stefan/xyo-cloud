...
class xyo_datasource_mysqli_Query extends xyo_Config {
	...
	function &copyThis() {
		$retV = new xyo_datasource_mysqli_Query($this->module_, $this->connection_, $this->name_, $this->datasource_, $this->descriptor_, false);
		if ($retV) {
	...
	function load($start=null, $length=null) {
		...
		if ($this->result_) {
			$fieldValue_ = $this->result_->fetch_assoc();
			if ($fieldValue_) {
				if(!empty($this->fieldSelect_)) {
					$this->setEmptyValue();
				};
	...
	function hasNext() {
		if ($this->result_) {
			$this->hasNextValue_ = $this->result_->fetch_assoc();
