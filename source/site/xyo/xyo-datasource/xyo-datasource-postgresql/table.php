...
class xyo_datasource_postgresql_Table extends xyo_Config {
	...
	function insert() {
		...
		if ($result) {
			if($this->fieldAutoIncrement_) {
				$query="SELECT CURRVAL(pg_get_serial_sequence('" . $this->realName_ . "','" . $this->fieldAutoIncrement_ . "'));";
	...
	function getQueryWhereClauseForFieldValue($field_as, $field_this, $value_) {
		if (is_array($value_)) {
			if (count($value_) == 1) {
				$value_ = $value_[0];
			} else {
	...
	function loadDirectCode($query) {
		...
		if ($this->result_) {
			$fieldValue_ = pg_fetch_assoc($this->result_);
			if ($fieldValue_) {
				if(!empty($this->fieldSelect_)) {
					$this->setEmptyValue();
				};
	...
	function hasNext() {
		if ($this->result_) {
			$this->hasNextValue_ = pg_fetch_assoc($this->result_);
	...
	function batch($what_=array()) {
		if(count($what_)) {
			foreach($what_ as $item_) {
				foreach($item_ as $key_=>$value_) {
					if(!is_array($value_)) {
						return false;
					};
	...
	function atomicAdd($field,$value) {
		...
		if($this->primaryKey_) {
			...
			if ($tablePrimaryKeyValue) {
				...
				$query = "BEGIN;";
				$query .= "SELECT \"".$field."\" FROM \"".$this->realName_."\" WHERE \"".$this->primaryKey_."\" = ".$tablePrimaryKeyValue." FOR UPDATE;";
				$query .= "UPDATE \"".$this->realName_."\" SET \"".$field."\" = \"".$field."\"+".$value." WHERE \"".$this->primaryKey_."\" = ".$tablePrimaryKeyValue.";";
				$query .= "COMMIT;";
	...
	function atomicIncrement($field){
		return $this->atomicAdd($field,1);
	}

	function atomicSub($field,$value) {
	...
	function atomicDecrement($field){
