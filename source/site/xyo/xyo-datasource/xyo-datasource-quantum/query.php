...
class xyo_datasource_quantum_Query extends xyo_Config {
	...
	function &copyThis() {
		$retV = new xyo_datasource_quantum_Query($this->module_, $this->connection_, $this->name_, $this->datasource_, $this->descriptor_, false);
		if ($retV) {
	...
	function prepareResult_() {
		...
		if (count($this->fieldOrder_)) {
			...
			uasort($this->resultRow_, "xyo_datasource_quantum_query__cmp");
			$this->resultRow_ = array_values($this->resultRow_);
		};
