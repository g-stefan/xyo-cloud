...
if ($this->xyo_module_group_id) {
	...
	if ($dsModuleGroup) {
		...
		if ($dsModuleGroup->load(0, 1)) {
			if ($this->xyo_module_id) {
				$this->setApplicationTitle($this->getFromLanguage("application.title") . " - " . $moduleName . " [". $dsModuleGroup->name. "]");
			} else {
				$this->setApplicationTitle($this->getFromLanguage("application.title") . " - " . $dsModuleGroup->name);
			};
        	};
	};
